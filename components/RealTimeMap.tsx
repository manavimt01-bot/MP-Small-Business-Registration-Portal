...
const DISTRICT_CENTERS: Record<string, { lat: number; lng: number; places: string[]; desc: string }> = {
  'Bhopal': { 
    lat: 23.2599, 
    lng: 77.4126, 
    places: ["New Market", "MP Nagar", "Bairagarh", "Kolar Road", "Lake View Drive"],
    desc: "Capital Administrative Node • High Intensity Congestion Watch"
  },
  'Indore': { 
    lat: 22.7196, 
    lng: 75.8577, 
    places: ["Sarafa Bazar", "Chhappan Dukan", "Rajwada Chowk", "Vijay Nagar", "Palasia Block"],
    desc: "Commercial Financial Hub • High Density Smart Vending Clusters"
  },
  'Ujjain': { 
    lat: 23.1760, 
    lng: 75.7885, 
    places: ["Mahakal Marg", "Freeganj Market", "Nanakheda Sector", "Shipra Waterfront Lane"],
    desc: "Pilgrimage Tourism Center • High Seasonality Street Operators"
  },
  'Gwalior': { 
    lat: 26.2183, 
    lng: 78.1828, 
    places: ["Bada Bazar", "Hazira Chowk", "Deen Dayal Nagar", "Fort View Row"],
    desc: "Heritage Tourism Buffer • Traditional Craft & Artisan Stalls"
  },
  'Jabalpur': { 
    lat: 23.1686, 
    lng: 79.9339, 
    places: ["Civic Center Market", "Wright Town Crossing", "Sadar Bazar Line", "Bhedaghat Enclave"],
    desc: "Narmada Riverine Sector • Active Micro-Finance Repayment Nodes"
  },
  'Sagar': { 
    lat: 23.8388, 
    lng: 78.7378, 
    places: ["Katra Bazar", "Civil Lines Crossing", "Gopal Ganj Lane"],
    desc: "Bundelkhand Central Corridor • Rural-Urban Enterprise Integration"
  },
  'Rewa': { 
    lat: 24.5363, 
    lng: 81.3033, 
    places: ["Sirmour Chowk", "Fort Road Area", "Jayanti Kunj Enclave"],
    desc: "Vindhya Agro-Produce Center • Dense Daily Produce Markets"
  },
  'Satna': { 
    lat: 24.5801, 
    lng: 80.8265, 
    places: ["Panni Lal Chowk", "Bharhut Nagar Sector", "Sadar Bazar Row"],
    desc: "Industrial Transit Junction • Dense Multi-Tiered Vendor Rows"
  }
};
...
export const RealTimeMap: React.FC<RealTimeMapProps> = ({ 
      ...
      setSimulatedVendorsMap(prev => {
        ...
        vendors.forEach(v => {
          if (v.id && prev[v.id]) {
            ...
            const isMobile = v.vendingType === 'mobile' || v.businessType.includes('ठेला') || v.businessType.includes('ঠেলা গাড়ী');
            if (isMobile && Math.random() < 0.6) {
              ...
              const center = DISTRICT_CENTERS[selectedDistrict] || DISTRICT_CENTERS['Bhopal'];
  ...
  const districtVendors = vendors.filter(v => {
    ...
    const fallbackMatch = selectedDistrict === 'Bhopal' && (v.location?.address || '').toLowerCase().includes('bhopal');
  ...
  const localMobileCarts = districtVendors.filter(v => v.vendingType === 'mobile' || v.businessType.includes('ठेला') || v.businessType.includes('ঠেলা')).length;
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-5 pb-5 border-b border-gray-200/50">
        <div>
          ...
          <h2 className="text-2xl font-black text-gray-900 tracking-tighter mt-2 leading-none">
            {t('ৰিয়েল-টাইম পৌৰ ভৌগোলিক মেপিং কন্সোল', 'Madhya Pradesh Municipal Geospatial Vending Console', 'नगरपालिका भू-स्थानिक मैपिंग कंसोल')}
          </h2>
        ...
        <div className="lg:col-span-8 flex flex-col justify-between space-y-4">
          ...
          >
            ...
            <svg className="absolute inset-0 w-full h-full opacity-30 pointer-events-none stroke-current text-slate-700/60" viewBox="0 0 100 100" preserveAspectRatio="none">
              ...
              {/* Secondary Son/Betwa Side Canal Waterway */}
            ...
            <div className="absolute inset-0 p-8 pointer-events-none z-10 text-[9px] font-mono leading-none font-bold uppercase select-none tracking-widest text-slate-600">
              ...
              <span className="absolute top-8 right-6 font-semibold italic text-slate-500 lowercase">Narmada Riverfront sector</span>
            </div>
