        ...
        (position) => {
          setProfile(prev => ({
            ...
            location: {
              ...
              address: prev.location?.address || t("न्यू मार्केट, भोपाल", "New Market, Bhopal", "न्यू मार्केट, भोपाल")
            }
          }));
        },
      ...
      setTimeout(() => {
        if (result && result.success) {
          const finalProfile: VendorProfile = { 
            ...
            dob: profile.dob,
            isVerified: false,
            id: `MP-${Math.random().toString(36).substring(2, 11).toUpperCase()}`,
            activeSchemes: [],
            loanStatus: 'none' as const,
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-16 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {successBanner && (
            <div className="p-5 bg-green-50 border border-green-100 rounded-[32px] flex items-center space-x-4 text-green-700 animate-in slide-in-from-top-4">
              <CheckCircle2 className="w-5 h-5 shrink-0 text-green-600" />
              <p className="text-xs sm:text-sm font-black leading-tight">{successBanner}</p>
            </div>
          )}
                  ...
                  <div className="flex-1 space-y-6 sm:space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1 block mb-1">
                            {t('ব্যৱসায়ৰ নাম (ঐচ্ছিক)', 'Business Name (Optional)', 'व्यवसाय का नाम (वैकल्पिक)')}
                          </label>
                          <input 
                            type="text"
                            placeholder={t("ব্যৱসায়ৰ নাম", "e.g. Raju Fruit Stall", "जैसे: राजू फ्रूट स्टॉल")}
                            className="w-full p-4 sm:p-5 bg-gray-50 border-3 border-transparent rounded-[20px] focus:bg-white focus:border-orange-500 transition-all font-bold text-sm sm:text-base shadow-sm"
                        ...
                        <div>
                          <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1 block mb-1">
                            {t('পেন কাৰ্ড নম্বৰ (ঐচ্ছিক)', 'PAN Card Number (Optional)', 'पैन कार्ड नंबर (वैकल्पिक)')}
                          </label>
                          <input 
                            type="text"
                            placeholder="ABCDE1234F"
                            maxLength={10}
                            className="w-full p-4 sm:p-5 bg-gray-50 border-3 border-transparent rounded-[20px] focus:bg-white focus:border-orange-500 transition-all font-bold text-sm sm:text-base shadow-sm uppercase tracking-wider font-mono"
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative aspect-square sm:aspect-video md:aspect-square bg-gray-950 rounded-[32px] overflow-hidden shadow-md border-4 border-white">
                          {!isCameraActive && !selfie ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-white/50 space-y-4 hover:bg-gray-900 transition-colors">
                              <button onClick={startCamera} className="flex flex-col items-center space-y-2 text-white/70 hover:text-white transition-colors cursor-pointer">
                              ...
                              <div className="text-gray-650 text-[10px] font-bold uppercase tracking-wider">{t('অথবা', 'OR', 'अथवा')}</div>
                              <label className="flex flex-col items-center space-y-1 cursor-pointer text-orange-400 hover:text-orange-350 transition-colors">
                                <Upload className="w-6 h-6" />
                                <span className="font-black text-[8px] sm:text-[9px] uppercase tracking-wider">{t('ফাইল আপলোড কৰক', 'Upload Photo', 'फ़ोटो अपलोड करें')}</span>
